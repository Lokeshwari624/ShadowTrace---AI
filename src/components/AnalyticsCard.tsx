...
const data = [
  ...
  { day: "Tue", scans: 68, threats: 14 },
  { day: "Wed", scans: 55, threats: 11 },
  { day: "Thu", scans: 89, threats: 22 },
  { day: "Fri", scans: 124, threats: 31 },
  { day: "Sat", scans: 98, threats: 19 },
  { day: "Sun", scans: 142, threats: 28 },
];

const AnalyticsCard = () => {
  const total = useMemo(() => data.reduce((s, d) => s + d.scans, 0), []);
  const trend = "+24.3%";
    ...
    >
      <header className="flex items-start justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Last 7 days · Live feed</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-success/10 border border-success/30">
          <TrendingUp className="h-3 w-3 text-success" />
          <span className="text-[10px] font-mono text-success">{trend}</span>
        </div>
      </header>

      <div className="grid grid-cols-3 gap-3 mb-4">
        <Stat label="Total Scans" value={total.toLocaleString()} accent="primary" />
        <Stat label="Threats" value="133" accent="destructive" />
        <Stat label="Resolved" value="87%" accent="success" />
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            ...
            <Area
              ...
              dot={{ fill: "hsl(189 94% 53%)", r: 3, strokeWidth: 0 }}
              activeDot={{ r: 5, stroke: "hsl(189 94% 53%)", strokeWidth: 2, fill: "hsl(222 47% 5%)" }}
            />
            <Area
              type="monotone"
              dataKey="threats"
              ...
              dot={{ fill: "hsl(258 90% 66%)", r: 2, strokeWidth: 0 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex items-center gap-4 mt-2 text-[11px] text-muted-foreground">
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-primary shadow-[0_0_6px_currentColor]" />
          Scans
        </div>
        <div className="flex items-center gap-1.5">
          <span className="h-2 w-2 rounded-full bg-accent shadow-[0_0_6px_currentColor]" />
          Threats
        </div>
